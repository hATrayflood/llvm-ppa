...
$(check-stamp): $(addprefix $(check-stamp)-,$(builds))
	# clang does not build all llvm binaries but looks for them in the build path
	# use the one from the Debian packages.
	LIST=$$(dpkg -L llvm-3.1|grep ^/usr/bin); \
	for f in $$LIST; do \
		TARGET=$(D)/build-clang/Release/bin/$$(basename $$f|sed -e "s|-3.1||g"); \
		if test ! -s $$TARGET; then \
			ln -s $$f $$TARGET; \
		fi; \
	done
	# Does not fail for now in case of errors
	-(cd $(D)/build-clang/tools/clang/test && make all)
	touch $@
...
$(configure-stamp)-%: $(unpack-stamp)
	...
	cd $($*_builddir) && \
	  ../$(srcdir)/configure $($*_confargs) CLANG_VENDOR=Debian	|| { cat config.log; exit 1; }
...
$(install-stamp)-%: $(build-stamp)-% check
